...
from tests import load_test_file_bytes


class TestXmlVerifier(unittest.TestCase):
    def test_empty_build(self):
        xml_string = load_test_file_bytes("resources/empty_build.xml")
    ...
    def test_ziz_build(self):
        xml_string = load_test_file_bytes("resources/ziz_huge_build.xml")
        valid_pob = is_valid_pob(xml_string)
        self.assertTrue(valid_pob, "Full blown build should be parsed as valid.")

    def test_malformed_build_with_missing_items(self):
        xml_string = load_test_file_bytes("resources/malformed_build_items_missing.xml")
        valid_pob = is_valid_pob(xml_string)
